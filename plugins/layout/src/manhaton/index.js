...
const manhatonLayout = (params) => {
  const {rankDir = 'RL', nodes, edges} = params.data;
  // 切片数组信息
  let piecesArray = [];
  // 计算node endpoint坐标
  const calcEndpointPos = (node) => {
    const endpoints = node.endpoints;
    endpoints.forEach(endpoint => {
      endpoint.left = node.left + (endpoint.orientation[0] / 2 + 0.5) * node.width;
      endpoint.top = node.top + (endpoint.orientation[1] / 2 + 0.5) * node.height;
    });
  }
  // 画布切片
  const canvasPieces = () => {
  ...
  const edgesAStar = () => {
    const getEndpointPiece = (endpoint) => {
      for (let cc = 0; cc < piecesArray.length; cc++) {
        // TODO 换向以及可优化
        if (_.get(piecesArray, `${cc}.0.left`) <= endpoint.left && _.get(piecesArray, `${cc + 1}.0.left`) > endpoint.left) {
          for (let dd = 0; dd < piecesArray[cc].length; dd++) {
            if (_.get(piecesArray, `${cc}.${dd}.top`) <= endpoint.top && _.get(piecesArray, `${cc}.${dd + 1}.top`) > endpoint.top) {
              return _.get(piecesArray, `${cc}.${dd}`);
            }
          }
        }
      }
      console.log([endpoint.left, endpoint.top]);
      throw Error('未找到endpoint对应的块!');
    };
    const edgeIdxSortlist = [];
    for (let cc of edges) {
      edgeIdxSortlist.push(cc.id);
    }
    edgeIdxSortlist.sort((aEdge, bEdge) => {
      const a = edges.find(e => e.id === aEdge);
      const b = edges.find(e => e.id === bEdge);
      // TODO 换向，并且可以更细化
      return -(Math.abs(nodes.find(n => n.id === b.sourceNode).left - nodes.find(n => n.id === b.targetNode).left) -
        Math.abs(nodes.find(n => n.id === a.sourceNode).left - nodes.find(n => n.id === a.targetNode).left))
    });
    for (let cc of edgeIdxSortlist) {
      const edge = edges.find(e => e.id === cc);
      const sourceNode = nodes.find(n => n.id === edge.sourceNode);
      const targetNode = nodes.find(n => n.id === edge.targetNode);
      const sourceEndpoint = sourceNode.endpoints.find(ele => ele.id === edge.source);
      const targetEndpoint = targetNode.endpoints.find(ele => ele.id === edge.target);
      ...
      edge.resultPath = simplifyResult(resultPath.reverse());
      // 一条边计算完,数据处理
  ...
  nodes.forEach(node => {
    calcEndpointPos(node);
  });

  console.log('endpoint计算 ' + (Date.now() - timer));
  timer = Date.now();

  piecesArray = canvasPieces();
  const piecesMap = [];
  for (let cc = 0; cc < piecesArray[0].length; cc++) {
    const result = [];
    for (let dd = 0; dd < piecesArray.length; dd++) {
      result.push(piecesArray[dd][cc].bounding < 100 ? '-' : '0');
    }
    piecesMap.push(result.join(""));
  }
  console.log('切片计算 ' + (Date.now() - timer));
  timer = Date.now();
  // console.log(piecesArray);
  // 根据节点寻路
  edgesAStar();
  console.log('A*计算 ' + (Date.now() - timer));
  timer = Date.now();
  return params;
}

class ManhatonEdge extends Edge {
  constructor(opts) {
    super(opts);
    this.resultPath = opts.options.resultPath;
    this.id = opts.options.id;
  }
  draw(obj) {
    let path = super.draw(obj);
    $(path).css('stroke', `rgb(${Math.random() * 255}, ${Math.random() * 255}, ${Math.random() * 255})`);
    return path;
  }
  calcPath(sourcePoint, targetPoint) {
    const result = this.resultPath;
